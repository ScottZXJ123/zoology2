...
sweep_name = f"hierarchical-hyena-test-{sweep_id}"
VOCAB_SIZE = 8_192
L_MAX = 64
D_MODEL = 64
N_LAYERS = 2
...
# Hyena
base_hyena_config = ModuleConfig(
...
for levels in [1, 2, 3]:
    models.append(ModelConfig(
        name=f"Hierarchical-Hyena-L{levels}-d{D_MODEL}",
        sequence_mixer=ModuleConfig(
            name="zoology.mixers.hierarchical.HierarchicalMixer",
            kwargs={"levels": levels, "mixer_config": base_hyena_config, "l_max": L_MAX}
        ),
        **model_factory_kwargs
    ))

models.append(ModelConfig(
    name=f"Baseline-Hyena-d{D_MODEL}",
    sequence_mixer=base_hyena_config,
    **model_factory_kwargs
))
