...
sweep_name = f"hierarchical-rwkv7-test-{sweep_id}"
VOCAB_SIZE = 8_192
L_MAX = 64
D_MODEL = 64
N_LAYERS = 2
...
# RWKV7
base_rwkv7_config = ModuleConfig(
...
for levels in [1, 2, 3]:
    models.append(ModelConfig(
        name=f"Hierarchical-RWKV7-L{levels}-d{D_MODEL}",
        sequence_mixer=ModuleConfig(
            name="zoology.mixers.hierarchical.HierarchicalMixer",
            kwargs={"levels": levels, "mixer_config": base_rwkv7_config, "l_max": L_MAX}
        ),
        **model_factory_kwargs
    ))
...
# 基线: 标准注意力
models.append(ModelConfig(
    name=f"Baseline-RWKV7-d{D_MODEL}",
    sequence_mixer=base_rwkv7_config,
    **model_factory_kwargs
))
