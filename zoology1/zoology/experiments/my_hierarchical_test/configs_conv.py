...
sweep_name = f"hierarchical-conv-test-{sweep_id}"
VOCAB_SIZE = 8_192
L_MAX = 64
D_MODEL = 64
N_LAYERS = 2
...
# BaseConv
base_conv_config = ModuleConfig(
    name="zoology.mixers.base_conv.BaseConv",
    kwargs={"l_max": L_MAX, "kernel_size": 3},
)

#循环创建您的分层模型
for levels in [1, 2, 3]:
    models.append(ModelConfig(
        name=f"Hierarchical-Conv-L{levels}-d{D_MODEL}",
        sequence_mixer=ModuleConfig(
            name="zoology.mixers.hierarchical.HierarchicalMixer",
            kwargs={"levels": levels, "mixer_config": base_conv_config, "l_max": L_MAX}
        ),
        **model_factory_kwargs
    ))
...
# 基线: 标准注意力
models.append(ModelConfig(
    name=f"Baseline-Conv-d{D_MODEL}",
    sequence_mixer=base_conv_config,
    **model_factory_kwargs
))
