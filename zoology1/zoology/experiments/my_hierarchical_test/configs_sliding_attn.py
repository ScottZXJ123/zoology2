...
sweep_name = f"hierarchical-sliding-attn-test-{sweep_id}"
VOCAB_SIZE = 8_192
L_MAX = 64
D_MODEL = 64
N_LAYERS = 2
...
base_sliding_attn_config = ModuleConfig(
    name="zoology.mixers.slide_attn.SlidingAttn",
    kwargs={"block_size": 128, "attention_dropout": 0.0},
)

#循环创建您的分层模型
for levels in [1, 2, 3]:
    models.append(ModelConfig(
        name=f"Hierarchical-SlidingAttn-L{levels}-d{D_MODEL}",
        sequence_mixer=ModuleConfig(
            name="zoology.mixers.hierarchical.HierarchicalMixer",
            kwargs={"levels": levels, "mixer_config": base_sliding_attn_config, "l_max": L_MAX}
        ),
        **model_factory_kwargs
    ))
...
# 基线: 标准注意力
models.append(ModelConfig(
    name=f"Baseline-SlidingAttn-d{D_MODEL}",
    sequence_mixer=base_sliding_attn_config,
    **model_factory_kwargs
))
