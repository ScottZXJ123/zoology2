# file: zoology/experiments/my_hierarchical_test/configs_gdn.py
...
sweep_name = f"hierarchical-gdn-test-{sweep_id}"
VOCAB_SIZE = 8_192
L_MAX = 64
D_MODEL = 64
N_LAYERS = 2
...
for levels in [1, 2, 3]:
    models.append(ModelConfig(
        name=f"Hierarchical-GDN-L{levels}-d{D_MODEL}",
        sequence_mixer=ModuleConfig(
            name="zoology.mixers.hierarchical.HierarchicalMixer",
            kwargs={"levels": levels, "mixer_config": base_gdn_config, "l_max": L_MAX}
        ),
        **model_factory_kwargs
    ))

models.append(ModelConfig(
    name=f"Baseline-GDN-d{D_MODEL}",
    sequence_mixer=base_gdn_config,
    **model_factory_kwargs
))
