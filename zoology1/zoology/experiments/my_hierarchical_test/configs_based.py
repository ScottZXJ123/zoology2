...
sweep_name = f"hierarchical-based-test-{sweep_id}"
VOCAB_SIZE = 8_192
L_MAX = 64
D_MODEL = 64
N_LAYERS = 2
...
# Based
base_based_config = ModuleConfig(
...
for levels in [1, 2, 3]:
    models.append(ModelConfig(
        name=f"Hierarchical-Based-L{levels}-d{D_MODEL}",
        sequence_mixer=ModuleConfig(
            name="zoology.mixers.hierarchical.HierarchicalMixer",
            kwargs={"levels": levels, "mixer_config": base_based_config, "l_max": L_MAX}
        ),
        **model_factory_kwargs
    ))

models.append(ModelConfig(
    name=f"Baseline-Based-d{D_MODEL}",
    sequence_mixer=base_based_config,
    **model_factory_kwargs
))
